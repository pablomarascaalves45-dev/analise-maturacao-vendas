...
def load_data(file):
    ...
    for col in cols_financeiras:
        if col in df.columns:
            # Se a coluna vier como texto (ex: "R$ 1.000,00"), limpamos
...
if uploaded_file:
    try:
        ...
        # Usando a coluna 29 e 30 como no seu padrão de DRE ou as colunas nomeadas
        total_prejuizo_mes = df['RO Mês'].sum()
        total_prejuizo_acum = df['RO Acum'].sum()
        # Média de aluguel (convertendo para escala 0-100 para exibição se necessário)
        ...
        with col_graf1:
            ...
            # As lojas com RO mais baixo (mais negativo)
