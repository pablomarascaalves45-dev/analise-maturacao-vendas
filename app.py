...
# ==============================================================================
# 5. DASHBOARD EXECUTIVO CONSOLIDAÇÃO (NOVA VERSÃO UNIFICADA)
# ==============================================================================
st.markdown("---")
st.header("5. Dashboard Executivo Consolidação")
st.sidebar.header("5. Dashboard Executivo")

if arquivos_dre and df_hist_global is not None:
    # --- ALTERAÇÃO AQUI: Criamos 3 colunas em vez de 2 ---
    col_sel1, col_sel2, col_sel3 = st.columns(3)
    
    with col_sel1:
        lista_nomes_dre = [f.name for f in arquivos_dre]
        dre_selecionado_nome = st.selectbox("Selecione o DRE para os KPIs:", lista_nomes_dre, key="sel_dre_exec")
    
    with col_sel2:
        # --- NOVO SELETOR DE LOJA ---
        try:
            # Carregamos o arquivo selecionado para ler os nomes das lojas (colunas)
            arq_temp = next(f for f in arquivos_dre if f.name == dre_selecionado_nome)
            df_temp = pd.read_excel(arq_temp, header=None)
            
            # Pegamos os nomes das lojas que geralmente começam da coluna 3 (índice D) em diante
            lojas_no_dre = [str(x).strip() for x in df_temp.iloc[0, 3:].dropna().unique()]
            loja_dre_selecionada = st.selectbox("Selecione a Unidade (DRE):", lojas_no_dre, key="sel_loja_dre")
            
            # Descobrimos o índice da coluna correspondente à loja selecionada
            # Somamos 3 porque o fatiamento acima começou da coluna 3
            col_loja_idx = 3 + lojas_no_dre.index(loja_dre_selecionada)
        except:
            col_loja_idx = 3 # Caso falhe, mantém o padrão da primeira loja disponível
            st.warning("Ajustando para coluna padrão.")

    with col_sel3:
        filiais_hist = sorted(df_hist_global['Desc_Filial'].unique())
        filial_exec_nome = st.selectbox("Selecione a Filial para o Gráfico:", filiais_hist, key="sel_filial_exec")

    try:
        # 1. PROCESSAMENTO FINANCEIRO (KPIs)
        arq_dre_exec = next(f for f in arquivos_dre if f.name == dre_selecionado_nome)
        df_exec = pd.read_excel(arq_dre_exec, header=None)
        
        # ... (Seu mapeamento de termos_ex continua igual)
        
        # --- AJUSTE NA FUNÇÃO DE VALOR ---
        # Agora ela usa 'col_loja_idx' em vez de um número fixo (3)
        def val_ex(k): return clean_numeric(df_exec.iloc[idx_ex[k], col_loja_idx]) if k in idx_ex else 0.0

        # O restante dos cálculos (v_rl, v_mc, v_res, etc) e a geração 
        # dos gráficos seguem exatamente como no seu código original.
