...
# 2. ENTRADA DE DADOS DE PROJEÇÃO (Lógica mantida, com melhorias de erro)
...
if arquivo_subido is not None:
    try:
        ...
        if colunas_disponiveis:
            ...
            with c1:
                fig = px.line(df_res, x="Mês", y="Faturamento", markers=True, title=f"Evolução - {estado_sel}")
            ...
            with c2:
                ...
                st.dataframe(df_res.style.format({"Faturamento": "R$ {:,.2f}", "% Maturação": "{:.2f}%"}), height=450)

    except Exception as e:
        st.error(f"Erro na Projeção: {e}")

# --- SEÇÃO DRE (CORREÇÃO DE FORMATO E EXIBIÇÃO DE MESES) ---
st.markdown("---")
...
if arquivo_dre is not None:
    try:
        # Lê sem pular linhas primeiro para achar o cabeçalho
        ...
        if not linhas_cab.empty:
            ...
            # Cards de Resumo
            vals = {k: pegar_v(v) for k, v in termos.items()}
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Faturamento Total", f"R$ {vals['RB']:,.2f}")
            c2.metric("Margem de Contrib.", f"R$ {vals['MC']:,.2f}")
            ...
            # --- TABELA DETALHADA COM MESES ---
            st.subheader("Tabela de Dados Financeiros Detalhada (Mensal)")
            
            # Remove colunas totalmente vazias
            df_exib = df_dre_raw.dropna(axis=1, how='all').fillna(0)
            
            # LÓGICA DE FORMATAÇÃO ROBUSTA (Evita o erro 'Unknown format code')
            fmt_dict = {}
            for col in df_exib.columns:
                # 1. Verifica se a coluna é numérica
                if pd.api.types.is_numeric_dtype(df_exib[col]):
                    # 2. Se for coluna de porcentagem
                    if any(x in col.upper() for x in ["AV-RI", "%", "META"]):
                        fmt_dict[col] = "{:.2%}"
                    # 3. Se for valores monetários (Realizado, Total ou nomes de Meses/Anos como 2025-01)
                    else:
                        fmt_dict[col] = "{:,.2f}"
                # Se não for numérica (texto), o Styler ignora automaticamente, evitando o erro.

            st.dataframe(
                df_exib.style.format(fmt_dict, na_rep="-"), 
